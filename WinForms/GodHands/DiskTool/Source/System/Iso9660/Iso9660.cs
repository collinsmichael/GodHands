...
namespace GodHands {
    ...
    public static partial class Iso9660 {
        ...
        public static bool Open(string path) {
            ...
            // volume parent must be null (for treeview)
            pvd = new Volume("CD:PVD", 0x10*2048);
            root = pvd.GetRootDir();
            //ReadFile(root);
                ...
                if (len == 0) {
                    ...
                    if (Records.ContainsKey(key)) {
                        ...
                        if (rec.FileFlags_Directory) {
                            Folder folder = new Folder(rec, rec.GetUrl()+":", rec.LenData);
                            Publisher.Register(folder);
                        } else {
                            Binary binary = new Binary(rec, rec.GetUrl()+":", rec.LenData);
