...
namespace GodHands {
    ...
    public static partial class RamDisk {
        ...
        public static bool SaveAs(string filepath) {
            ...
            try {
                byte[] buffer = new byte[2352];
                FileStream stream = File.Create(filepath, 65536);
                Logger.Pass("Created file "+filepath);
                
                int head = offset;
                int body = 2048;
                int tail = 2352 - (head + body);

                for (int lba = 0; lba < RamDisk.count; lba++) {
                    file.Position = lba*2352;
                    file.Read(buffer, 0, 2352);
                    stream.Position = lba*2352;
                    stream.Write(buffer, 0, 2352);
                    Buffer.BlockCopy(buffer, offset, disk, lba*2048, 2048);
                    map[lba] = 0x78;
                    if (lba % 4096 == 0) {
                        int progress = lba*100/count;
                        Logger.SetProgress(progress);
                        Logger.Info("Writing sector "+lba+" of "+count);
                    }
                }
                ...
                Logger.Pass("Saved as "+path);
            } catch (Exception e) {
                IsBusy = false;
                return Logger.Fail("File not created! "+e.Message);
            }
            IsBusy = false;
            return true;
        }
